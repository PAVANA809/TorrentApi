...
// SSE-enabled search that streams live log messages to the client
async function SearchWithLogs(searchQuery, logFn, cancelSignal) {
    for (const provider of providers) {
        if (cancelSignal.cancelled) throw new Error('Search cancelled by user');
        try {
            logFn('INFO', `Trying provider: ${provider}`);
            TorrentSearchApi.enableProvider(provider);
            let allTorrents = [];
            for (const category of categories) {
                if (cancelSignal.cancelled) throw new Error('Search cancelled by user');
                logFn('INFO', `Searching for "${searchQuery}" in category "${category}" on ${provider}`);
                const torrents = await TorrentSearchApi.search(searchQuery, category, 20);
                logFn('INFO', `Found ${torrents.length} torrents from ${provider} (${category})`);
            ...
            if (torrentsWithImdb.length > 0) {
                logFn('SUCCESS', `Found ${torrentsWithImdb.length} torrents with IMDB info from ${provider}`);
                try {
                    torrentsWithImdb[0]["imgurl"] = await scrapeImdb.getImdbImg(torrentsWithImdb[0].imdb);
                } catch (imgErr) {
                    logFn('WARN', `Could not fetch IMDB image: ${imgErr.message}`);
                }
            }
            ...
            if (prioritizedTorrents.length > 0) {
                ...
                logFn('WARN', `No torrents found from ${provider}`);
            }
        } catch (error) {
            if (cancelSignal.cancelled) throw error;
            logFn('ERROR', `Error fetching from ${provider}: ${error.message || error}`);
        }
    }
    logFn('ERROR', 'All providers failed or returned no results.');
    return [];
}
...
// SSE endpoint: streams live logs + final results to the frontend
app.get('/search/stream/:query', async (req, res) => {
    const query = req.params.query;
    console.log(`[SSE] /search/stream/${query}`);
    ...
    // Log function passed into search
    function logFn(level, message) {
        const ts = new Date().toISOString().replace('T', ' ').slice(0, 19);
        const logLine = `[${ts}] [${level}] ${message}`;
        console.log(logLine);
        sendEvent('log', { level, message, timestamp: ts });
    }

    // Cancel if client disconnects
    req.on('close', () => {
        cancelSignal.cancelled = true;
        console.log(`[SSE] Client disconnected, cancelling search for: ${query}`);
    });

    try {
        sendEvent('log', { level: 'INFO', message: `Starting search for "${query}"...`, timestamp: new Date().toISOString().replace('T', ' ').slice(0, 19) });
        const data = await SearchWithLogs(query, logFn, cancelSignal);

        if (cancelSignal.cancelled) {
            sendEvent('cancelled', { message: 'Search was cancelled by user.' });
        } else if (!data || data.length === 0) {
            sendEvent('error', { error: 'No torrents found or all providers failed (may be blocked on this host).' });
        } else {
            sendEvent('log', { level: 'SUCCESS', message: `Search complete! Returning ${data.length} results.`, timestamp: new Date().toISOString().replace('T', ' ').slice(0, 19) });
            sendEvent('results', { data });
        }
    } catch (error) {
        if (cancelSignal.cancelled) {
            sendEvent('cancelled', { message: 'Search was cancelled by user.' });
